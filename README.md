...
下面是一些从维护者/贡献者角度对仓库的说明：
- 考虑到本校物理实验每年教学内容，ppt和教材可能的变化（甚至每个助教讲的都不一样），表格公式的可编辑性和可移植性会较为重要，也方便外校学生fork后自行改编。
- 关于保留小数的问题，有三种方式可用，首先不打算直接采用保留小数位数的数字格式/单元格格式，可编辑性太差了。第二种是用ROUND函数在excel中保留小数数位，第三种是直接在表头写明保留几位但实际输出还是原来本的浮点小数，本人更倾向于第三种做法（因为懒，而且这样能始终保持浮点输出不舍入，确保精度够高）。
- 本仓库暂时不打算包含实验报告样例。
